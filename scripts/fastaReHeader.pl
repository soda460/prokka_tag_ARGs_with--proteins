...
open (OUTFILE, ">$outdir/$dna_id/$last_path_part") or die "Impossible d'ecrire un fichier de sortie a cet endroit";
...
while (my $seq = $inseq->next_seq) {
	...
	#>Res13-Abat-PEA21-P4-01-A.fasta|Contig_71_119.351
	if ($fasta_id =~ /Contig/) {
		$case = 3;		
		#print ("2018-2019vPHAC header (after MobSuite)\n");
		&guess_header($case, $fasta_id, $file, $seq);
		next;
	}
...
sub guess_header{
	...
	if ($case == 3) {
		...
		if ($fasta_id =~ /(Contig\_)(\d+)/) {
			my $newHeader = ('>'.$mol_name.'_contig'. $2. "\n");
			my $newHeader2 = substr $newHeader, 0, 36;
			print OUTFILE $newHeader2; 
			print OUTFILE ($seq->seq, "\n");
			return
